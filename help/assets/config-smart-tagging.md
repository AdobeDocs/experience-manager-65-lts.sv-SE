...
1. Välj **[!UICONTROL Smart Content Services]** på sidan **[!UICONTROL Select product profiles]**. Klicka på **[!UICONTROL Save configured API]**.
   ...
   På en sida visas mer information om konfigurationen. Håll den här sidan öppen för att kopiera och lägga till dessa värden i [!UICONTROL Assets Smart Tagging Service Settings] av molnkonfigurationen i [!DNL Experience Manager] för att konfigurera smarta taggar.
...
1. Gå till **[!UICONTROL Tools]** > **[!UICONTROL Security]** > **[!UICONTROL Adobe IMS Configurations]** i användargränssnittet för [!DNL Experience Manager].
...
1. Åtkomst till din [!DNL Experience Manager]-server på `https://[aem_server]:[port]`.
...
1. Klicka på `validateConfigs()`. Klicka på **[!UICONTROL Invoke]** i dialogrutan **[!UICONTROL Validate Configurations]**.
...
1. Expandera sidopanelen för att visa stegen. Dra **[!UICONTROL Smart Tag Asset]**-steget som är tillgängligt i DAM-arbetsflödesavsnittet och placera det efter **[!UICONTROL Process Thumbnails]**-steget.

   ![Lägg till resurssteget för smarta taggar efter steget med processminiatyrbilder i arbetsflödet DAM-uppdatering](assets/smart-tag-in-dam-update-asset-workflow.png)

1. Öppna egenskaperna för steget för att ändra informationen. Kontrollera att alternativet **[!UICONTROL Handler Advance]** är markerat under **[!UICONTROL Advanced Settings]**.
...
**Fullständighet:** Om en bild kvalificerar för mer än en tagg lägger du till alla tillämpliga taggar innan du inkluderar bilden för utbildning. För taggar, till exempel `raincoat` och `model-side-view`, lägger du till båda taggarna i den kvalificerade resursen innan du inkluderar den för utbildning.
...
1. Ange en rubrik och beskrivning för rapporten. Under **[!UICONTROL Schedule Report]** låter du alternativet **[!UICONTROL Now]** vara markerat. Om du vill schemalägga rapporten för senare väljer du **[!UICONTROL Later]** och anger ett datum och en tid. Klicka sedan på **[!UICONTROL Create]** i verktygsfältet.
1. På sidan **[!UICONTROL Asset Reports]** väljer du rapporten som du skapade. Om du vill visa rapporten klickar du på **[!UICONTROL View]** i verktygsfältet.
1. Granska informationen i rapporten.

   Rapporten visar utbildningsstatusen för de taggar du har tränat. Den gröna färgen i kolumnen **[!UICONTROL Training Status]** anger att Smart Content Service har tränats för taggen. Gul färg anger att tjänsten inte är helt tränad för en viss tagg. I det här fallet lägger du till fler bilder med den särskilda taggen och kör utbildningsarbetsflödet för att utbilda tjänsten helt på taggen.
