...
* **filePattern**: Anger inkommande mönster för bifogade filer som accepteras av providern. Detta inkluderar filer som har specifika filnamnstillägg (&amp;ast;.dat, &amp;ast;.xml), filer som har specifika namn (data) och filer som har sammansatta uttryck i namnet och tillägget (&amp;ast;.`[dD][aA]`&#39;port&#39;). Standardvärdet är `*`.
...
**Mappningstyp**: Används för att konfigurera indatavärden som krävs för att anropa tjänståtgärden. Två typer av mappningar är:
