...
När du återger HTML-formulär som innehåller både sidnavigeringsfunktioner och formulärskript (t.ex. anta att ett skript hämtar fältdata från en databas varje gång formuläret återges) kontrollerar du att formulärskriptet har formen :calculate i stället för formuläret :readyevent.

Formulärskript i formen :ready körs bara en gång under den inledande återgivningen av formuläret och körs inte för efterföljande sidhämtningar. Däremot körs händelsen :calculate för varje sidnavigering där formuläret återges.
