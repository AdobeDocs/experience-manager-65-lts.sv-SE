...
* **FORMS-24688:** När JBoss EAP 8.0 konfigureras i Linux kan gränssnittsskript som ändrats i Windows innehålla CRLF-radslut, vilket kan orsaka `/bin/sh^M: bad interpreter` eller $`\r`: det gick inte att hitta fel. Lös problemet genom att konvertera filer som påverkas till LF-radslut med dos2unix.

* **FORMS-24689:** I uppgraderingsläget för inaktuell version kanske inte uppdatering av databasporten i `lc_turnkey.xml` tillämpas korrekt under uppgraderingen, vilket gör att processen refererar till det gamla portvärdet.
