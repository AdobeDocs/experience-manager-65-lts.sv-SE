...
* **FORMS-24688:** När JBoss EAP 8.0 (AEM Forms 6.5.1 LTS) konfigureras i Linux kan skript som ändrats i Windows innehålla CRLF-radslut, vilket orsakar /bin/sh^M: felaktig tolk eller $&#39;\r&#39;: fel hittades inte. Konvertera de påverkade .sh-filerna till Unix-format (LF) med hjälp av dos2unix (till exempel sök. -type f -name &quot;*.sh&quot; -exec dos2unix {} +) för att lösa problemet.

* **FORMS-24689:** I uppgraderingsläget för körklar nyckel plockas inte uppdateringar av databasporten från användargränssnittet och måste ändras manuellt i konfigurationsfilen lc_körkey.xml. Om det inte uppdateras korrekt kan systemet fortsätta att använda det gamla portvärdet.

* **FORMS-24687:** I AEM Forms 6.5.1 LTS-klusterdistributioner på JBoss EAP 8 kan `domain/configuration/domain_oracle.xml` innehålla en dubbletttagg, vilket resulterar i ogiltig XML som förhindrar att domänkontrollanten startar.`<security>`
